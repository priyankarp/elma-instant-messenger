...
namespace messenger {
    ...
    std::pair<std::string,std::string> MessengerClient::url_parts(std::string url) {
    ...
    MessengerClient& MessengerClient::get(std::string url, std::function<void(json&)> handler) {
        std::thread t(&MessengerClient::_get_thread,this,url,handler);
        t.detach(); // detaching means we don't have to join later
    }

    MessengerClient& MessengerClient::post(std::string url, json data, std::function<void(json&)> handler) {
        // TODO: problem 2
    ...
    MessengerClient& MessengerClient::process_responses() {
    ...
    const std::shared_ptr<httplib::Response> MessengerClient::_get_aux(std::string url) {
    ...
    const std::shared_ptr<httplib::Response> MessengerClient::_post_aux(std::string url, json data) {
        auto parts = url_parts(url);
    ...
    void MessengerClient::_get_thread(std::string url, std::function<void(json&)> handler) {
    ...
    void MessengerClient::_post_thread(std::string url, json data, std::function<void(json&)> handler) {
