#ifndef _ELMA_MSGR_CLIENT_H
#define _ELMA_MSGR_CLIENT_H
...
namespace messenger {
    ...
    class MessengerClient {
        ...
        MessengerClient& get(std::string url, std::function<void(json&)> handler);

        // Problem 2
        MessengerClient& post(std::string url, json data, std::function<void(json&)> handler);

        //! Process all responses received so far
        //! \return A reference to the client, for chaining        
        MessengerClient& process_responses();
